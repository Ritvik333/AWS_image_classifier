...
variable "s3_bucket_name" {
  description = "S3 bucket for images and code"
  type        = string
...
resource "aws_cognito_user_pool" "pool" {
  ...
  # Enable email as a user attribute
  schema {
    name                = "email"
  ...
  # Email configuration (using Cognito's default email sender)
  email_configuration {
...
resource "aws_cognito_user_pool_client" "client" {
  name         = "ImageRecognitionClient"
  user_pool_id = aws_cognito_user_pool.pool.id

  # Allow the client to use the AUTHORIZATION_CODE, IMPLICIT, and REFRESH_TOKEN flows
  allowed_oauth_flows                  = ["code", "implicit"]
  allowed_oauth_flows_user_pool_client = true
  allowed_oauth_scopes                 = ["email", "openid", "profile"]
  callback_urls                        = ["http://localhost:3000"]  # Update for dev
  logout_urls                          = ["http://localhost:3000"]  # Update for dev
  supported_identity_providers         = ["COGNITO"]

  # Generate a client secret
  generate_secret = true

  # Explicitly enable USER_PASSWORD_AUTH flow
  explicit_auth_flows = [
    "USER_PASSWORD_AUTH"
  ]
}
...
# Update the POST /upload Method to Require Authentication
...
# Update the GET /results/{taskId} Method to Require Authentication
